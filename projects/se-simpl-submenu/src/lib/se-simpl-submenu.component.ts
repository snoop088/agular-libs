...
import { SeSimplSubmenuOptions } from './se-simpl-submenu-options';
...
@Component({
  selector: 'se-simpl-submenu',
  template: `
    <ng-container *ngIf="submenuState === 'opened'">
...
export class SeSimplSubmenuComponent implements OnInit, AfterContentInit {

  // Options Input to set up the SubMenu behaviour
  @Input() set options(options: SeSimplSubmenuOptions) {
    if (options) {
  ...
  @ContentChildren(SeSimplSubmenuItemDirective) menuItemsQueryList: QueryList<SeSimplSubmenuItemDirective>;

  private tl: TimelineMax;
  private _submenuState: SubmenuState;
  private _options: SeSimplSubmenuOptions;
